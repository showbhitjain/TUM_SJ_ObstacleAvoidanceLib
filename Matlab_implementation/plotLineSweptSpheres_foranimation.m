function h = plotLineSweptSpheres_foranimation(linesegments, ax)
     axis equal; grid on;
    ...
    h = gobjects(length(linesegments) * 2, 1); % Prepare handles for two plots per segment ('a' and 'd')
    idx = 1;
    ...
    for i = 1:length(linesegments)
        ...
        if ~any(isnan(linesegments(i).aSegmentV0)) && ~any(isnan(linesegments(i).aSegmentV1))
            ...
            idx = idx + 1;
            h(idx) = plotSphere_foranimation(linesegments(i).aSegmentV0, linesegments(i).radius, 'Parent', ax);
            idx = idx + 1;
        end
        ...
        if ~any(isnan(linesegments(i).dSegmentV0)) && ~any(isnan(linesegments(i).dSegmentV1))
            ...
            idx = idx + 1;
            h(idx) = plotSphere_foranimation(linesegments(i).dSegmentV0, linesegments(i).radius, 'Parent', ax);
            idx = idx + 1;
        end
    end

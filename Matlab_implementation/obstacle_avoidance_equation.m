...
for l = 1:length(obstacles)
    ...
    for i = 1:length(linesegments)
        ...
        if criticalPoints(i,l).has_criticalpoints_D && criticalPoints(i,l).has_criticalpoints_A
            criticalPoints(i,l).distance = min(dist_roc_A,dist_roc_D);
        end
...
b_0_calculate = zeros(maxEntries, 1);
...
for l = 1:length(obstacles)
    ...
    if config.obstacle_avoidance_scheme1
        for i = 1:length(linesegments)
            ...
            if ~any(isnan(criticalPoints(i,l).roc_A))
                ...
                b_g_calculate(b_counter + (1:3)) = smoothing_value .* max((criticalPoints(i,l).jacobi_dist_max_A * (criticalPoints(i,l).jointVelocity_critical_A)'),0);
            ...
            if ~any(isnan(criticalPoints(i,l).roc_D))
                ...
                b_g_calculate(b_counter + (1:3)) = smoothing_value .* max((criticalPoints(i,l).jacobi_dist_max_D * (criticalPoints(i,l).jointVelocity_critical_D)'),0);
                b_counter = b_counter + 3;
            end
    ...
    if config.obstacle_avoidance_scheme2
        for i = 1:length(linesegments)
            ...
            if ~any(isnan(criticalPoints(i,l).jacobi_distance_A))
                J_0_calculate(J_counter + 1, :) = -criticalPoints(i,l).roc_A * criticalPoints(i,l).jacobi_critical_A;
                J_counter = J_counter + 1;
            end

            if ~any(isnan(criticalPoints(i,l).jacobi_distance_D))
                J_0_calculate(J_counter + 1, :) = -criticalPoints(i,l).roc_D *  criticalPoints(i,l).jacobi_critical_D;
                J_counter = J_counter + 1;
            end

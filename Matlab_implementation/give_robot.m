function robot_manipulator = give_robot(mdhparams,Homejointpositions,jointMinValues,jointMaxValues,Gripper_position,Gripper_orientation)
%Create a rigid body tree. This tree is initialized with a base coordinate frame to attach bodies to.
    number_of_joints = size(mdhparams,1);
    robot = rigidBodyTree(MaxNumBodies= 50,DataFormat="row");
    ...
    bodies = cell(number_of_joints-1,1);
    joints = cell(number_of_joints-1,1);
    for i = 2:number_of_joints
        [bodies{i-1}, joints{i-1}] = createRigidBodyAndJoint(sprintf('robot_link%d', int32(i)), mdhparams(i,:), Homejointpositions(i), [jointMinValues(i), jointMaxValues(i)]);
    end
    ...
    %     body.Joint = joint;
    %     addBody(robot, body, sprintf('robot_link%d', int32(i-1)));
    % end
     
    for i=1:number_of_joints-1
        addBody(robot,bodies{i},sprintf('robot_link%d', int32(i-1)));    
    end
    ...
    addBody(robot,body_Tcp,sprintf('robot_link%d', int32(number_of_joints)));
    robot_manipulator = robot;
end

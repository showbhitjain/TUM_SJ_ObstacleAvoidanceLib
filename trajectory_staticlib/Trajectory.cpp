...
// = orientationTrajectory(orientations,waypointTimes,ts,trajType)
//
...
// Return Type  : void
//
void Trajectory::orientationTrajectory(
    ...
    ::coder::array<double, 2U> &desired_angular_accel)
{
  ...
  // 'orientationTrajectory:9' trajTimes = 0:ts:waypointTimes(end);
  b = std::isnan(ts);
  if (b) {
    nm1d2 = 1;
  } else {
  ...
  // 'orientationTrajectory:11' desired_quaternions = zeros(4,numel(trajTimes));
  desired_quaternions.set_size(4, nm1d2);
  loop_ub = nm1d2 << 2;
  for (i = 0; i < loop_ub; i++) {
    desired_quaternions[i] = 0.0;
  }
  // 'orientationTrajectory:12' desired_angular_velocity =
  // zeros(3,numel(trajTimes));
  desired_angular_velocity.set_size(3, nm1d2);
  loop_ub_tmp = 3 * nm1d2;
  // 'orientationTrajectory:13' desired_angular_accel =
  // zeros(3,numel(trajTimes));
  ...
  // 'orientationTrajectory:14' numWaypoints = size(orientations,2);
  //  Loop through trajectory generation
  // 'orientationTrajectory:18' for w = 1:numWaypoints-1
  ...
  for (int w{0}; w <= i - 2; w++) {
    ...
    int i1;
    boolean_T result;
    // 'orientationTrajectory:20' if size(orientations,1) == 3
    if (orientations.size(0) == 3) {
      double c_data_idx_2;
      double s_data_idx_1;
      // 'orientationTrajectory:21' R0 =
      ...
      // 'orientationTrajectory:23' elseif size(orientations,1) == 4
      // 'orientationTrajectory:24' R0 = quaternion(orientations(:,w)')  ;
      ...
      // 'orientationTrajectory:25' Rf = quaternion(orientations(:,w+1)');
      ...
      // 'orientationTrajectory:26' else
      // 'orientationTrajectory:27' error('Invalid Orientation')
    }
    // 'orientationTrajectory:29' timeInterval = waypointTimes(w:w+1);
    timeInterval[0] = waypointTimes[w];
    timeInterval[1] = waypointTimes[w + 1];
    //  Recalculate trajectory times for the segment
    // 'orientationTrajectory:32' trajTimes_rot =
    ...
    // 'orientationTrajectory:33' num_samples = round((timeInterval(2) -
    // timeInterval(1))/ts) + 1;
    //  Use appropriate trajectory generation method
    // 'orientationTrajectory:36' switch trajType
    result = false;
    ...
    switch (loop_ub_tmp) {
    case 0:
      // 'orientationTrajectory:37' case 'trap'
      // 'orientationTrajectory:38' [s,sd,sdd] = trapveltraj([0 1],num_samples,
      // ... 'orientationTrajectory:39' 'AccelTime',diff(timeInterval)/3, ...
      // 'orientationTrajectory:40' 'EndTime',diff(timeInterval)*2/3);
      d = c_data_idx_1 - waypointTimes[w];
      coder::trapveltraj(std::round(d / ts) + 1.0, d / 3.0, d * 2.0 / 3.0, s,
                         sd, sdd);
      break;
    case 1:
      // 'orientationTrajectory:41' case 'cubic'
      // 'orientationTrajectory:42' [s,sd,sdd] = cubicpolytraj([0
      // 1],timeInterval,trajTimes_rot);
      coder::cubicpolytraj(timeInterval, trajTimes_rot, s, sd, sdd);
      break;
    case 2:
      // 'orientationTrajectory:43' case 'quintic'
      // 'orientationTrajectory:44' [s,sd,sdd] = quinticpolytraj([0
      // 1],timeInterval,trajTimes_rot);
      coder::quinticpolytraj(timeInterval, trajTimes_rot, s, sd, sdd);
      break;
    default:
      // 'orientationTrajectory:45' otherwise
      // 'orientationTrajectory:46' error('Invalid trajectory type! Use
      // ''trap'', ''cubic'', or ''quintic''');
      break;
    }
    //  Generate the rotation trajectory
    // 'orientationTrajectory:50' [R, omega, alpha] = rottraj(R0, Rf,
    ...
    // 'orientationTrajectory:53' start_idx = round(timeInterval(1)/ts) + 1;
    ndbl = std::round(waypointTimes[w] / ts);
    // 'orientationTrajectory:54' end_idx = round(timeInterval(2)/ts) + 1;
    cdiff = std::round(c_data_idx_1 / ts);
    //  Insert data
    // 'orientationTrajectory:57' desired_quaternions(:, start_idx:end_idx) =
    // compact(R)';
    ...
    // 'orientationTrajectory:58' desired_angular_velocity(:, start_idx:end_idx)
    // = omega;
    if (ndbl + 1.0 > cdiff + 1.0) {
      nm1d2 = 0;
      i1 = 0;
    ...
    // 'orientationTrajectory:59' desired_angular_accel(:, start_idx:end_idx) =
    // alpha;
    if (ndbl + 1.0 > cdiff + 1.0) {
      nm1d2 = 0;
      i1 = 0;
...
//
// function [xd, xd_vel, xdd] = positionTrajectory(waypoints, waypointTimes, ts,
// trajType, waypointVels, waypointAccels)
//
...
// Return Type  : void
//
void Trajectory::positionTrajectory(
    ...
    ::coder::array<double, 2U> &xd_vel, ::coder::array<double, 2U> &xdd)
{
  ...
  // 'positionTrajectory:18' if nargin < 5 || isempty(waypointVels)
  if (waypointVels.size(1) == 0) {
    // 'positionTrajectory:19' waypointVels = 0.1 * zeros(3, size(waypoints,
    // 2));
  ...
  // 'positionTrajectory:21' if nargin < 6 || isempty(waypointAccels)
  if (waypointAccels.size(1) == 0) {
    // 'positionTrajectory:22' waypointAccels = zeros(size(waypointVels));
  ...
  // 'positionTrajectory:26' waypointAccelTimes = 0.25 * diff(waypointTimes);
  ...
  //  Define trajectory sample times
  // 'positionTrajectory:29' trajTimes = 0:ts:waypointTimes(end);
  guard1 = false;
  if (std::isnan(ts)) {
    guard1 = true;
  } else {
  ...
  // 'positionTrajectory:30' coder.varsize("xd",[3 Inf],[0 1]);
  // 'positionTrajectory:31' coder.varsize("xd_vel",[3 Inf],[0 1]);
  // 'positionTrajectory:32' coder.varsize("xdd",[3 Inf],[0 1]);
  //  Switch between trajectory types and generate the desired trajectory
  // 'positionTrajectory:36' switch trajType
  result = false;
  if (trajType.size(1) == 4) {
    ncols = 0;
    do {
  ...
  switch (ncols) {
  case 0:
    // 'positionTrajectory:37' case 'trap'
    // 'positionTrajectory:38' [xd, xd_vel, xdd] = trapveltraj(waypoints,
    // numel(trajTimes), ... 'positionTrajectory:39' 'AccelTime',
    // repmat(waypointAccelTimes, [3, 1]), ... 'positionTrajectory:40'
  ...
  case 1: {
    ...
    int jcol;
    // 'positionTrajectory:41' case 'cubic'
    // 'positionTrajectory:42' [xd, xd_vel, xdd] = cubicpolytraj(waypoints,
    // waypointTimes, trajTimes, ... 'positionTrajectory:43'
  ...
  case 2:
    // 'positionTrajectory:44' case 'quintic'
    // 'positionTrajectory:45' [xd, xd_vel, xdd] = quinticpolytraj(waypoints,
    // waypointTimes, trajTimes, ... 'positionTrajectory:46'
    // 'VelocityBoundaryCondition', waypointVels, ... 'positionTrajectory:47'
    ...
    break;
  default:
    // 'positionTrajectory:48' otherwise
    // 'positionTrajectory:49' error('Invalid trajectory type! Use ''trap'',

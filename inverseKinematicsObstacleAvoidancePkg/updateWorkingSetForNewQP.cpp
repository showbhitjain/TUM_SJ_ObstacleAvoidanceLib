                              ...
                              const array<double, 2U> &ub, int mFixed)
{
  ...
  hasLB = ((lb.size(0) != 0) && (lb.size(1) != 0));
  hasUB = ((ub.size(0) != 0) && (ub.size(1) != 0));
  if (hasLB) {
  ...
  if (hasLB && hasUB) {
    for (int idx{0}; idx < mFixed; idx++) {
      double d;
      d = xk[WorkingSet.indexFixed[idx] - 1];
      WorkingSet.ub[WorkingSet.indexFixed[idx] - 1] =
          ub[WorkingSet.indexFixed[idx] - 1] - d;
      WorkingSet.bwset[idx] = ub[WorkingSet.indexFixed[idx] - 1] - d;
    }
  }
  i = mFixed + mEq;

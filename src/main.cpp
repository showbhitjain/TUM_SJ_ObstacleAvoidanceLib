...
tuple<MatrixXd, MatrixXd, MatrixXd> positionTrajectory(ConfigurationParameters const &trajConfig) {
    ...
    return Trajectoryptr->positionTrajectory(Waypoints, WaypointTimes, ts, positionTrajectoryType, waypointVelocities,
...
//    auto controlled_stepper = make_controlled<Stepper>(1e-6, 1e-6);
    ...
    for (int i = 0; i < trajTimes.size()-1; i++) {
        ...
        auto [optimalJointVelocity, ExitFlag] = ik.ikWithConstraints(actualJointValuesMatrix(all, i), JacobiMatrix,
...
//        double dt = (tEnd - tStart) / 10;
        ...
        actualJointValuesMatrix(all, i+1) = addRandomNoisetoJointsSignal(desiredJointValuesMatrix(all, i+1),-0.0005,0.0005);
        cout<<"desired Joint Value: \n"<< desiredJointValuesMatrix(all, i+1)<<endl;
    }

    cout << "actualJointValues:  \n" << actualJointValuesMatrix(all, 1458) << endl;
    writeMatrixToCSV("../output.csv",actualJointValuesMatrix);

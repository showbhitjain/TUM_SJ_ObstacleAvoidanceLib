...
Obstacles::calculateDistanceRobotLinkObstacle(Eigen::Vector3d const &linkSegmentV0,
...
//caluclate closest points and distance between two spheres
std::tuple<double, Eigen::Vector3d, Eigen::Vector3d>
Obstacles::calculateDistanceSphereSphere(double const &radius1, Eigen::Vector3d const &center1, double const &radius2,
...
// cylinderAxis is  normalized
//return sequence: distance magnitude, sphere closest point, cylinder closest point
std::tuple<double, Eigen::Vector3d, Eigen::Vector3d>
Obstacles::calculateDistanceSphereCylinder(double const &radiusSphere, Eigen::Vector3d const &sphereCenter,
    ...
    Vector3d cylinderStartVertex = cylinderCenter + cylinderAxis * (-cylinderHeight / 2);
    Vector3d cylinderEndVertex = cylinderCenter + cylinderAxis * (cylinderHeight / 2);
    ...
    if (projection <= 0) {
        //closest point on Cylinder lies on the circle with starting vertix of axis as center
        s = 0;
        closestPointCylinderAxis = cylinderStartVertex;
        distanceVector = sphereCenter - closestPointCylinderAxis;
        double distanceVectorNormalProjectedMagnitude = distanceVector.dot(-cylinderAxis);
        auto distanceVectorNormalProjected = distanceVectorNormalProjectedMagnitude * (-cylinderAxis);
        ...
        distanceVector = (sphereCenter - closestPointCylinderAxis).normalized();
    ...
    double distanceMagnitude = (closestPointSphere - closestPointCylinder).norm();
    if ((sphereCenter - closestPointCylinder).norm < radiusSphere)
    {
...
std::tuple<double, Eigen::Vector3d, Eigen::Vector3d>
Obstacles::calculateDistanceSphereBox(double const &radiusSphere, Eigen::Vector3d const &sphereCenter,
...
std::tuple<double, Eigen::Vector3d, Eigen::Vector3d>
Obstacles::calculateDistanceBoxBox(Eigen::Vector3d const &boxCenter1, Eigen::Vector3d const &boxDimensions1,
...
std::tuple<double, Eigen::Vector3d, Eigen::Vector3d>
Obstacles::calculateDistanceBoxCylinder(Eigen::Vector3d const &boxCenter, Eigen::Vector3d const &boxDimensions,
    ...
    // -------------------------------------------------------------
    fcl::Cylinderd fclCylinder(radiusCylinder, cylinderHeight / 2.0);
 ...
 */
std::tuple<double, Eigen::Vector3d, Eigen::Vector3d>
Obstacles::calculateDistanceCylinderCylinder(double radiusCylinder1, double cylinderHeight1,
    ...
    // -----------------------------
    double halfLen1 = cylinderHeight1 * 0.5;
    double halfLen2 = cylinderHeight2 * 0.5;
    fcl::Cylinderd cylGeom1(radiusCylinder1, halfLen1);
    fcl::Cylinderd cylGeom2(radiusCylinder2, halfLen2);

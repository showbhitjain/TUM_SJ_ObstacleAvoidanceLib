...
DQ_SerialManipulatorMDH Robot::createRobotMDH_from_Path(const std::string &configFile_Path, const std::string &parameterFor, const std::string &whichrobot,const AndreiUtils::Posed& baseFrame){
auto Config_file = ConfigurationParameters(configFile_Path,parameterFor); //get the file which has path to the robotconfigfile
Config = Config_file.getSubConfig(whichrobot); // create config file from class Configurationparameters for the type of robot
...
Robot::Robot(const std::string &configFile_Path, const std::string &parameterFor, const std::string &whichrobot,const AndreiUtils::Posed& baseFrame): robotmdh(createRobotMDH_from_Path(configFile_Path,parameterFor,whichrobot,baseFrame)){
    auto Config_file = ConfigurationParameters(configFile_Path,parameterFor); //get the file which has path to the robotconfigfile
    this->Config = Config_file.getSubConfig(whichrobot); // create config file from class Configurationparameters for the type of robot
    auto s = createRobotMDH(Config,fromPoseToDQ(baseFrame));
    auto ro = make_shared<DQ_SerialManipulatorMDH>(s);
    this->robot = ro;
    this->joints = make_shared<Joints>(Config);
...
Eigen::VectorXd Robot::fkm_cartesian(const Eigen::VectorXd &jointValues, const int& to_ith_link) const{
...
Eigen::MatrixXd Robot::get_transformationmatrix(const Eigen::VectorXd &jointValues, const int& to_ith_link) const {
    int i = to_ith_link -1;
...
Eigen::MatrixXd Robot::jacobian_cartesian(const Eigen::VectorXd &jointValues, const int &to_ith_link) const {
    ...
    Eigen::Matrix4d T_end_effector = get_transformationmatrix(jointValues, to_ith_link);
    Eigen::Vector3d end_effector_position = T_end_effector.block<3, 1>(0, 3);


    for (int i = 1; i <= to_ith_link; ++i) {
        // Get the transformation matrix from the base frame to the ith joint

            Eigen::Matrix4d T_i = get_transformationmatrix(jointValues, i);

      ...
      if (idx <= nVar - 1) {
        ...
        if (workingset.probType == 2) {
          ...
          if (workingset.Aeq.size(0) != 0) {
            ...
            iAcol = workingset.nVarOrig + workingset.sizes[1];
            for (idx = 0; idx <= iy; idx++) {
        ...
        if (workingset.probType == 2) {
          ...
          if (workingset.Aeq.size(0) != 0) {
            ...
            for (idx = 0; idx <= iy; idx++) {
              workingset.maxConstrWorkspace[idx] =
                  (workingset.maxConstrWorkspace[idx] -
                   workspace[(br + workingset.nVarOrig) + idx]) +
                  workspace[(br + iAcol) + idx];
        ...
        if (workingset.sizes[3] > 0) {
          for (idx = 0; idx < jBcol; idx++) {
            v = std::fmax(v, -workspace[(br + workingset.indexLB[idx]) - 1] -
                                 workingset.lb[workingset.indexLB[idx] - 1]);
          }
        }
        if (workingset.sizes[4] > 0) {
          for (idx = 0; idx < ar; idx++) {
            v = std::fmax(v, workspace[(br + workingset.indexUB[idx]) - 1] -
        ...
        if (workingset.sizes[0] > 0) {
          for (idx = 0; idx < mFixed; idx++) {
            v = std::fmax(
                v, std::abs(workspace[(br + workingset.indexFixed[idx]) - 1] -
                            workingset.ub[workingset.indexFixed[idx] - 1]));
          }
        }

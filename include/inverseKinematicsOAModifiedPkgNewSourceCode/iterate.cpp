      ...
      if (subProblemChanged) {
          ...
          for (idx = qrmanager.mrows - 2; idx + 2 > qrmanager.ncols; idx--) {
            ...
            if (qrmanager.mrows >= 1) {
              ...
              for (k = 0; k < n; k++) {
                iyend = b_iy + k;
                ix0 = iy + k;
                temp = c * qrmanager.Q[ix0] + s * qrmanager.Q[iyend];
                qrmanager.Q[iyend] =
                    c * qrmanager.Q[iyend] - s * qrmanager.Q[ix0];
        ...
        if (solution.state != -5) {
          ...
          temp = feasibleratiotest(
              ...
              workingset.nWConstr, TYPE == 5, updateFval, i, ix0);
          if (updateFval) {
            switch (i) {
            case 3:
              workingset.nWConstr[2]++;
              workingset.isActiveConstr[(workingset.isActiveIdx[2] + ix0) - 2] =
                  true;
              workingset.nActiveConstr++;
              workingset.Wid[workingset.nActiveConstr - 1] = 3;
              workingset.Wlocalidx[workingset.nActiveConstr - 1] = ix0;
              iyend = workingset.ldA * (ix0 - 1);
              ...
              workingset.bwset[workingset.nActiveConstr - 1] =
                  workingset.bineq[ix0 - 1];
              break;
            case 4:
              WorkingSet::addBoundToActiveSetMatrix_(workingset, 4, ix0);
              break;
            default:
              WorkingSet::addBoundToActiveSetMatrix_(workingset, 5, ix0);
              break;
            }
            activeSetChangeID = 1;
          } else {

    ...
    double config_weightManipulability)
{
  ...
  if (config_useObjectiveNormInfinity) {
    ...
    if (current_joint_velocity.size(0) == 0) {
      ...
      for (int k{0}; k < i; k++) {
        ...
        if (std::isnan(absx) || (absx > t306)) {
          t306 = absx;
        }
      }
    }
    objVal = config_weightNormInfinity * (t306 * t306) / 2.0;
  }
  ...
  if (config_useObjectiveTrajectoryFollowing) {
    ...
    for (int k{0}; k < pipk; k++) {
      aoffset = k * 6;
      absx += jacobi[aoffset] * current_joint_velocity[k];
      t380 += jacobi[aoffset + 1] * current_joint_velocity[k];
      t389 += jacobi[aoffset + 2] * current_joint_velocity[k];
      t395 += jacobi[aoffset + 3] * current_joint_velocity[k];
      t111 += jacobi[aoffset + 4] * current_joint_velocity[k];
      t188 += jacobi[aoffset + 5] * current_joint_velocity[k];
    }
    scale = 3.3121686421112381E-170;
    absx = std::abs(absx - xd_eff_vel[0]);
    ...
    absx = std::abs(t380 - xd_eff_vel[1]);
    if (absx > scale) {
      t = scale / absx;
      t306 = t306 * t * t + 1.0;
      scale = absx;
    ...
    absx = std::abs(t389 - xd_eff_vel[2]);
    if (absx > scale) {
      t = scale / absx;
      t306 = t306 * t * t + 1.0;
      scale = absx;
    ...
    absx = std::abs(t395 - xd_eff_vel[3]);
    if (absx > scale) {
      t = scale / absx;
      t306 = t306 * t * t + 1.0;
      scale = absx;
    ...
    absx = std::abs(t111 - xd_eff_vel[4]);
    if (absx > scale) {
      t = scale / absx;
      t306 = t306 * t * t + 1.0;
      scale = absx;
    ...
    absx = std::abs(t188 - xd_eff_vel[5]);
    if (absx > scale) {
      t = scale / absx;
      t306 = t306 * t * t + 1.0;
      scale = absx;
    ...
    t306 = scale * std::sqrt(t306);
    objVal += config_weightTrajectoryFollowing * (t306 * t306) / 2.0;
  }
  ...
  if (config_useObjectiveJointAcceleration) {
    ...
    if (current_joint_velocity.size(0) == starting_joint_vel.size(0)) {
      ...
      if (static_cast<int>(current_joint_velocity.size(0) < 200)) {
        for (int i1{0}; i1 < pipk; i1++) {
          b[i1] = current_joint_velocity[i1] - starting_joint_vel[i1];
        }
      } else {
#pragma omp parallel for num_threads(                                          \
  ...
  if (config_useObjectiveManipulability) {
    ...
    if (static_cast<int>(joint_values.size(0) < 200)) {
      for (int i1{0}; i1 < pipk; i1++) {
        b_joint_values[i1] = joint_values[i1];
      }
    } else {
#pragma omp parallel for num_threads(                                          \
    4 > omp_get_max_threads() ? omp_get_max_threads() : 4)
    ...
    b_t10[158] = t274;
    b_t10[159] = scale * 6.123233995736766E-17;
    ...
    b_t10[168] = scale * 5.0516680464828323E-18;
    ...
    b_t10[240] = t395;
    b_t10[241] = scale * 3.749399456654644E-33;
    b_t10[242] = -t395;
    ...
    b_t10[248] = scale * 4.011857418620469E-34;
...
#pragma omp parallel for num_threads(                                          \
    4 > omp_get_max_threads() ? omp_get_max_threads() : 4) private(d, i2)

    for (int i1 = 0; i1 < 6; i1++) {
      for (i2 = 0; i2 < 6; i2++) {
        d = (((((J[i1] * J[i2] + J[i1 + 6] * J[i2 + 6]) +
                J[i1 + 12] * J[i2 + 12]) +
               J[i1 + 18] * J[i2 + 18]) +
              J[i1 + 24] * J[i2 + 24]) +
             J[i1 + 30] * J[i2 + 30]) +
    ...
    absx = inv_JJt[0] * inv_JJt[7] * inv_JJt[14] * inv_JJt[21] * inv_JJt[28] *
           inv_JJt[35];
    if (ipiv[4] > 5) {
      isodd = !isodd;
    }
    ...
    if (ipiv[3] > 4) {
      pipk = p[ipiv[3] - 1];
      p[ipiv[3] - 1] = p[3];
      p[3] = static_cast<signed char>(pipk);
    }
    if (ipiv[4] > 5) {
      pipk = p[ipiv[4] - 1];
      p[ipiv[4] - 1] = p[4];
      p[4] = static_cast<signed char>(pipk);
    }
    for (int k{0}; k < 6; k++) {
      inv_JJt_tmp = 6 * (p[k] - 1);
    ...
    for (int b_i{0}; b_i < 7; b_i++) {
      ...
      for (i = 0; i < 6; i++) {
        ...
        for (aoffset = 0; aoffset < 6; aoffset++) {
          pipk = aoffset + 42 * b_i;
          JJt[i + 6 * aoffset] =
              (((((absx * H[pipk] + t * H[pipk + 6]) + t306 * H[pipk + 12]) +
                 t380 * H[pipk + 18]) +
                t389 * H[pipk + 24]) +
               t395 * H[pipk + 30]) +
              t111 * H[pipk + 36];
      ...
      //  Equation (11)
      scale += -(t188 * absx) * current_joint_velocity[b_i];
    }
